...
* book_slug MUST remain unique.
* category_slug MUST remain unique.
* publication_year MUST support historical book publication dates.
* borrowed_stock MUST always synchronize automatically with active borrowings.
* available_stock + borrowed_stock MUST NEVER exceed total_stock.
...
* qr_serial_number MUST remain globally unique.
...
* late_penalty_amount MUST support decimal currency precision.
...
* member_qr_uuid MUST remain globally unique.
* class_name MUST support customizable school-specific formats.
...
notification delivery retries MUST be supported.

Data Integrity:
* is_read MUST default to false.
...
* The backend MUST support database connection pooling.
...
* Rate-limited requests MUST return standardized API responses.
* Distributed rate limiting MUST support Redis-backed storage.
* List APIs MUST support pagination, filtering, and sorting.
* The system MUST implement centralized error handling.
* The backend MUST provide health check endpoints (e.g., `/health`, `/api/v1/health`).
* Health check endpoints MUST support database and Redis connectivity verification.
* The system MUST support structured application logging.
* Critical failures MUST be logged automatically.
...
* Audit logs MUST support chronological event tracing.
...
* affected_record_id MUST support flexible primary key reference formats.
* old_value and new_value MUST support structured JSON storage.
* school uniqueness MUST consider regional hierarchy.
...
All timestamps MUST use UTC timezone storage.
...
* Uploaded files MUST use secure randomized filenames.
* Direct executable access to uploaded files MUST be prevented.
...
* Passwords MUST enforce configurable password complexity policies.
...
* The backend MUST support configurable CORS policies.
* Production deployments MUST enforce HTTPS connections.
* Realtime socket connections MUST require authenticated sessions.
...
* unique index qr_serial_number
* unique index isbn_code
* unique index member_qr_uuid
* unique index book_code
* unique index borrowing_code
* unique index email_address
* unique index student_id_number
